...
const smsUnreg = () => { return `\`\`\`¡¡NECESITAS ESTAR REGISTRADO PARA USAR ESTE COMANDO, ESCRIBE #verificar PARA REGISTRARTE!!\`\`\`` }
const smsRestrict = () => { return `\`\`\`¡¡ESTE COMANDO ESTÁ RESTRINGIDO POR MI CREADOR!!\`\`\`` }

//main.js
const smsWelcome = () => { return `*╭┈⊰* @subject *⊰┈ ✦*\n*┊✨ BIENVENIDO!!*\n┊🔝 @user\n┊📄 *LEA LA DESCRIPCIÓN DEL GRUPO*\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✦*\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ *@user*\n┊ *NO FUE DIGNO DE ESTAR AQUÍ!!* 🌟\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
...
const smsOfc1 = () => { return '🤑 *◜CUENTA OFICIAL◞* 🤑'}
const smsOfc2 = () => { return `*❥ _Sigueme en mi Cuenta Oficial para mantenerte al tanto del_ ${packname}*`}

//info-gruposofc.js
const smsGrupoOfc1 = () => { return '*◜😏 GRUPO OFICIAL 😏◞*'}
...
const smsContacto4 = () => { return 'Contacto Oficial del SharkLite 🦈'}
...
const smsPropdesban4 = (number, usr) => { return `${lenguajeGB.smsAvisoAG()}*@${number} ESTAS DESBANEADO POR @${usr} YA PUEDES USAR LOS COMANDOS!!* 🪄`}
...
//_antilink.js
const smsAdwa = () => { return `${global.lenguajeGB['smsAvisoEG']()}*COMO ES ADMIN NO SERÁ ELIMINADO*`}
const smsEnlaceWat = () => { return `${lenguajeGB['smsAvisoAG']()}*SE DETECTÓ UN ENLACE DE WHATSAPP!!*\n\n*SERÁS ELIMINADO*`}
const smsWaMismoEnlace = () => { return '*ESTE ENLACE ES DE ESTE GRUPO, SÓLO POR ESO NO SERÁ ELIMINADO*'}

//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeGB['smsAvisoAG']()}*SE DETECTÓ UN ENLACE QUE CONTIENE HTTPS!!*\n\n*SERÁS ELIMINADO*`}

//_allantilink.js 
const smsTextoYT = () => { return '𝗦𝘂𝗽𝗲𝗿 𝙎𝙝𝙖𝙧𝙠𝙇𝙞𝙩𝙚 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const smsSoloOwner = () => { return `*MÍ PROPIETARIO DEBE DE ACTIVAR LA FUNCIÓN DE RESTRINGIR*`}
...
const smsHandlerLlamar = (tagUserL, llamadaVideo) => { return `*HOLA @${tagUserL} ${llamadaVideo ? 'LAS VÍDEOLLAMADAS 📲' : 'LAS LLAMADAS 📞'} ESTÁ PROHIBIDO EN ESTE CHAT. SERÁS BLOQUEADO*`} 
...
const smsIAimage2 = () => { return 'Un tiburon de color morado con celeste estando en Júpiter, iluminando el cosmo con su encanto con un efecto minimalista.'} 
